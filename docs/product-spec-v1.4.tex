...
\vspace{0.5cm}
\noindent VAL-4 is covered by automated tests: \texttt{tests/unit/test\_etim\_loader.py} (10 tests) and \texttt{tests/integration/test\_etim\_real\_files.py} (1 test, against the real release files). Both require a database fixture and are run locally rather than in CI. VAL-5 is specified but not yet executable: the ETIM matching stages are designed and not built, so the test exists as a specification only. This is recorded rather than deferred silently.
